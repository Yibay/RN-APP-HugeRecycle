...
class MyOrder extends Component {
	...
	render() {
		return (
			<View style={styles.container}>
				<Header title='我的订单' navigation={this.props.navigation} />
				{/* 回收物品栏 */}
	...
	componentDidMount() {
		AsyncStorage.getItem('X-AUTH-TOKEN')
			.then(token => request.get(config.api.base + config.api.order + this.props.navigation.state.params.id, null, {'X-AUTH-TOKEN': token}))
			.then(res => {
				console.log(res);
				this.setState({
					order: res.data
